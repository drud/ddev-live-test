...
GITHUB_REPO="ddev-demo/my-wordpress-${1}-site"
SITE_BASENAME="wp${1}"
VERSION=$(sed 's/./&./1' <<< "${1}")
...
set -x
time ddev-live create site wordpress "${SITENAME}" --github-repo="${GITHUB_REPO}" --run-composer-install --docroot web --wordpress-version="${VERSION}" --php-version=7.3 --persistent-paths=app/uploads
set +x
elapsed
...
time ddev-live push files "${SITENAME}" assets/"${SITE_BASENAME}" app/uploads/ >/tmp/filespush."${SITENAME}" 2>&1
