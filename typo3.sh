...
GITHUB_REPO="ddev-demo/my-typo3-${1}-site"
SITE_BASENAME="t${1}"
...
set -x
time ddev-live create site typo3 "${SITENAME}" --github-repo="${GITHUB_REPO}" --run-composer-install --docroot public --typo3-version="${1}" --php-version=7.3
set +x
elapsed
...
time ddev-live push files "${SITENAME}" assets/"${SITE_BASENAME}" fileadmin/ >/tmp/filespush."${SITENAME}" 2>&1
